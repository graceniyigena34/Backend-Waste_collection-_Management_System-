...
export const createCompanyDriver = async (
  ...
  const result = await pool.query(
    `INSERT INTO company_drivers (company_id, name, phone, email, license_number, national_id, years_of_experience)
     VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *`,
    [companyId, data.name, data.phone, data.email ?? null, data.license_number ?? null, data.national_id ?? null, data.years_of_experience ?? 0],
  );
